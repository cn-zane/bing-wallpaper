...
def print_hi():
    ...
    res_html = requests.get("https://cn.bing.com/HPImageArchive.aspx?format=js&idx=0&n=3&uhd=1", headers=headers)
    res = res_html.content.decode("utf-8")
    json_res = json.loads(res)
    for num in range(3):
        url = json_res["images"][num]["url"]
        # url截取
        url = url[:url.find('&')];
        picture_title = json_res["images"][num]["copyright"]
        end_date = json_res["images"][num]["enddate"]
        # 截取标题
